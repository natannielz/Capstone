...
import {matchesOrderQueue, orderFulfillmentLabel, orderValue, relatedOrderInvoices, substitutionNeedsDecision} from "@/lib/domain/order-views";
...
function CustomerOrderDetail(ctx: Context & {id: string}) {
  ...
  const substitutions = s.substitutions.filter(sub => lines.some(l => l.id === sub.orderLineId) && substitutionNeedsDecision(s, sub));
  ...
  return <>
    ...
    {substitutions.map(sub => {const product = get(s.products, sub.productId); return <section className="customer-section" key={sub.id}><h2>Toko mengusulkan barang pengganti</h2><p>{product.name} · {money(sub.price)} per {product.unit}</p><p>{sub.reason}</p><div className="customer-actions">{[true, false].map(approve => <Button key={String(approve)} variant={approve ? "default" : "outline"} onClick={() => ask({title: approve ? "Setujui barang pengganti" : "Tolak barang pengganti", description: `${product.name}, ${money(sub.price)} per ${product.unit}. Keputusan ini berlaku untuk sisa barang yang diusulkan toko.`, type: "substitution.decide", data: {id: sub.id, approve}, fields: [], label: approve ? "Setujui pengganti" : "Tolak pengganti"})}>{approve ? "Setujui" : "Tolak"}</Button>)}</div></section>;})}
...
function CustomerInvoice(ctx: Context & {invoice: Invoice; shared: boolean}) {
  ...
  return <div className="customer-invoice"><header className="customer-section-heading"><div><h3>{invoice.number}</h3><p>Jatuh tempo {dateLabel(invoice.dueDate)}</p></div><Badge variant="secondary">{invoiceStatus(s, invoice)}</Badge></header><dl className="customer-amounts"><div><dt>Total tagihan</dt><dd>{money(invoiceTotal(s, invoice.id))}</dd></div><div><dt>Sisa pembayaran</dt><dd>{money(balance)}</dd></div></dl>{shared && <p>Invoice ini menggabungkan penerimaan dari beberapa pesanan Anda.</p>}
    ...
    {s.credits.filter(c => c.invoiceId === invoice.id).map(c => <p className="customer-notice" key={c.id}>Koreksi tagihan {money(c.amount)} · {c.status === "approved" ? "Disetujui" : "Menunggu persetujuan"}. {c.reason}</p>)}
  </div>;
}
...
function CustomerPayment(ctx: Context & {payment: Payment}) {
  ...
  return <article className="customer-payment"><header><strong>{money(p.amount)}</strong><Badge variant="outline">{p.status === "recorded" ? "Menunggu verifikasi" : allocated > 0 ? "Dialokasikan" : "Terverifikasi"}</Badge></header><p>{p.reference || (p.kind === "credit" ? "Saldo koreksi tagihan" : "Transfer tanpa referensi")} · {dateLabel(p.date)}</p>{p.status === "verified" && <p>{money(allocated)} telah dialokasikan · {money(paymentAvailable(s, p.id))} saldo tersedia</p>}{s.refunds.filter(r => r.paymentId === p.id).map(r => <p key={r.id}>Pengembalian {money(r.amount)} · {r.status === "paid" ? "Dibayar secara simulasi" : r.status === "approved" ? "Disetujui" : "Diajukan"}. {r.reason}</p>)}{p.kind !== "credit" && <CustomerAttachments {...ctx} scope="payment" targetId={p.id}/>}</article>;
}

function CustomerAttachments({s, refresh, scope, targetId}: Context & {scope: "payment" | "receipt"; targetId: string}) {

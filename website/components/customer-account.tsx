...
import {customerOrderActions, customerOrderSectionId, customerPaymentLabel, customerProductHref, customerSubstitutionViews, safeCustomerOrderListHref} from "@/lib/domain/customer-order-views";
...
export function CustomerAccount({initialActor, view, orderId}: {initialActor: Actor; view: "profile" | "orders"; orderId?: string}) {
  ...
  const [errorSource, setErrorSource] = useState<"refresh" | "logout">("refresh");
  ...
  const refresh = useCallback(() => gate(async () => {
    ...
    try {
      ...
      setErrorSource("refresh"); setError(cause instanceof Error ? cause.message : "Data belum dapat dimuat. Coba kembali.");
  ...
  return <ShopShell actor={actor} active={view === "profile" ? "account" : "orders"}>
    <div className="customer-account account-v14">
      ...
      <section className="customer-account-main" id="customer-account-content" aria-label="Isi akun pelanggan">
        ...
        {ctx && loading && <p className="customer-refresh-status" role="status">Memperbarui data akun…</p>}
        {error && <Alert variant="destructive"><AlertDescription>{error} <Button variant="link" onClick={() => void (errorSource === "logout" ? logout() : refresh())} disabled={loading || logoutBusy}>{errorSource === "logout" ? "Coba keluar lagi" : "Coba kembali"}</Button></AlertDescription></Alert>}
        ...
        {!ctx && loading && <div className="customer-loading" role="status" aria-label="Memuat akun"><Skeleton className="h-24 w-full"/><Skeleton className="h-60 w-full"/><span className="sr-only">Memuat data akun…</span></div>}
...
function CustomerOrders({s}: Context) {
  ...
  const actionCounts = new Map(s.orders.map(order => [order.id, customerOrderActions(s, order.id).length]));
  const needsActionCount = s.orders.filter(order => (actionCounts.get(order.id) || 0) > 0).length;
  const orders = [...s.orders].filter(o => (queue === "needs-pic" ? (actionCounts.get(o.id) || 0) > 0 : queue === "preparation" ? ["submitted", "approved"].includes(o.status) && ["Menunggu tinjauan", "Diproses", "Terpenuhi sebagian"].includes(orderFulfillmentLabel(s, o)) : matchesOrderQueue(s, o, queue)))
  ...
  const backHref = safeCustomerOrderListHref(url({page: String(page)}));
  const detailHref = (id: string) => `/account/orders/${encodeURIComponent(id)}?from=${encodeURIComponent(backHref)}`;
  const credit = sum(s.payments.map(p => paymentAvailable(s, p.id)));
  return <>
    {needsActionCount > 0 && <Link className="customer-task-banner" href={url({status: "needs-pic", page: ""})}><Package aria-hidden="true"/><span><strong>{needsActionCount} pesanan perlu tindakan Anda</strong><span>Periksa penerimaan, usulan pengganti, atau pembayaran.</span></span><ArrowRight aria-hidden="true"/></Link>}
    <div className="customer-status-links" aria-label="Filter status pesanan">{Object.entries(queues).map(([key, label]) => <Link key={key} href={url({status: key, page: ""})} aria-current={key === queue ? "page" : undefined}>{label}{key === "needs-pic" && needsActionCount > 0 && <span className="customer-filter-count">{needsActionCount}</span>}</Link>)}</div>
    ...
    <div className="customer-order-list">{orders.slice((page - 1) * 8, page * 8).map(order => <article className="customer-order-card" key={order.id}>
      <header><div><Link href={detailHref(order.id)}><strong>{order.number}</strong></Link><span>{dateLabel(order.createdAt)}</span></div><div className="customer-card-status"><Badge variant="secondary">{orderFulfillmentLabel(s, order)}</Badge>{Boolean(actionCounts.get(order.id)) && <span className="customer-action-count">{actionCounts.get(order.id)} tindakan</span>}</div></header>
      <OrderProducts s={s} order={order} preview/>
      <footer><div><span>Nilai pesanan</span><strong>{money(orderValue(s, order.id))}</strong></div><Button asChild variant="outline"><Link href={detailHref(order.id)}>Lihat pesanan<ArrowRight data-icon="inline-end"/></Link></Button></footer>
    </article>)}</div>
    {orders.length > 8 && <nav className="customer-pagination" aria-label="Halaman pesanan">{page <= 1 ? <Button variant="outline" disabled>Sebelumnya</Button> : <Button asChild variant="outline"><Link href={url({page: String(page - 1)})}>Sebelumnya</Link></Button>}<span>{page} / {Math.ceil(orders.length / 8)}</span>{page >= Math.ceil(orders.length / 8) ? <Button variant="outline" disabled>Berikutnya</Button> : <Button asChild variant="outline"><Link href={url({page: String(page + 1)})}>Berikutnya</Link></Button>}</nav>}
  </>;
}
...
function CustomerOrderDetail(ctx: Context & {id: string}) {
  ...
  const params = useSearchParams();
  const backHref = safeCustomerOrderListHref(params.get("from"));
  const order = s.orders.find(o => o.id === id);
  if (!order) return <Empty><EmptyHeader><EmptyTitle>Pesanan tidak tersedia</EmptyTitle><EmptyDescription>Pesanan tidak ditemukan atau tidak dapat diakses akun ini.</EmptyDescription></EmptyHeader><Button asChild variant="outline"><Link href={backHref}>Kembali ke pesanan</Link></Button></Empty>;
  ...
  const actions = customerOrderActions(s, id);
  const events = [
    {id: "created", label: "Pesanan dibuat", date: order.createdDate || order.createdAt},
    ...(order.reviewedDate ? [{id: "reviewed", label: order.status === "rejected" ? "Pesanan ditolak" : "Ditinjau toko", date: order.reviewedDate}] : []),
    ...(order.cancelledDate ? [{id: "cancelled", label: "Sisa pesanan dibatalkan", date: order.cancelledDate}] : []),
    ...shipments.flatMap(shipment => [
      {id: `${shipment.id}-prepared`, label: `${shipment.number} · Pengiriman disiapkan`, date: shipment.date},
      ...(shipment.receivedDate ? [{id: `${shipment.id}-received`, label: `${shipment.number} · Barang diterima`, date: shipment.receivedDate}] : []),
      ...(shipment.failedDate ? [{id: `${shipment.id}-failed`, label: `${shipment.number} · Gagal dikirim`, date: shipment.failedDate}] : []),
      ...(shipment.cancelledDate ? [{id: `${shipment.id}-cancelled`, label: `${shipment.number} · Pengiriman dibatalkan`, date: shipment.cancelledDate}] : []),
    ]),
    ...invoices.map(({invoice}) => ({id: invoice.id, label: `${invoice.number} · Tagihan diterbitkan`, date: invoice.date})),
  ...
  return <>
    <Link className="customer-back" href={backHref}><ArrowLeft aria-hidden="true"/>Kembali ke daftar pesanan</Link>
    <div className="customer-order-overview"><div><h2>{order.number}</h2><p>Dibuat {dateLabel(order.createdAt)}</p></div><Badge variant="secondary">{orderFulfillmentLabel(s, order)}</Badge></div>
    {actions.length > 0 && <section className="customer-next-actions" aria-labelledby="customer-next-title"><header><div><h2 id="customer-next-title">Perlu tindakan Anda</h2><p>Selesaikan langkah yang sesuai dengan kondisi pesanan.</p></div><span>{actions.length} tindakan</span></header><ul>{actions.map(action => <li key={`${action.kind}-${action.id}`}><a href={action.href}><span className="customer-next-icon">{action.kind === "payment" || action.kind === "proof" ? <WalletCards aria-hidden="true"/> : <Package aria-hidden="true"/>}</span><span><strong>{action.title}</strong><span>{action.detail}</span></span><ArrowRight aria-hidden="true"/></a></li>)}</ul></section>}
    <section className="customer-section"><header className="customer-section-heading"><div><h2>Barang pesanan</h2><p>Rincian barang dan nilai setelah pembatalan.</p></div></header>
    ...
    {substitutions.map(({substitution: sub, originalProduct, replacementProduct, quantity, originalTotal, total, needsDecision, status}) => <section className="customer-section" key={sub.id} id={customerOrderSectionId("substitution", sub.id)} tabIndex={-1}>
    ...
    <section className="customer-section"><h2>Perjalanan pesanan</h2><ol className="customer-timeline">{events.map(event => <li key={event.id}><strong>{event.label}</strong><span>{dateLabel(event.date)}</span></li>)}</ol></section>
    {shipments.map(sh => <CustomerShipment key={sh.id} {...ctx} shipment={sh}/>)}
    <section className="customer-section"><h2>Tagihan & pembayaran</h2>{!invoices.length ? <p>Tagihan diterbitkan setelah barang diterima dan penerimaan diselesaikan toko. Anda belum perlu membayar pesanan ini.</p> : invoices.map(({invoice, shared, orderAmount}) => <CustomerInvoice key={invoice.id} {...ctx} invoice={invoice} shared={shared} orderAmount={orderAmount}/>)}</section>
...
function CustomerInvoice(ctx: Context & {invoice: Invoice; shared: boolean; orderAmount: number}) {
  const {s, actor, ask, invoice, shared, orderAmount} = ctx;
  ...
  return <div className="customer-invoice" id={customerOrderSectionId("invoice", invoice.id)} tabIndex={-1}><header className="customer-section-heading"><div><h3>{invoice.number}</h3><p>Jatuh tempo {dateLabel(invoice.dueDate)}</p></div><Badge variant="secondary">{invoiceStatus(s, invoice)}</Badge></header>{shared && <p className="customer-invoice-share"><strong>{money(orderAmount)}</strong> dari tagihan ini berasal dari pesanan yang sedang Anda lihat. Total di bawah mencakup beberapa pesanan Anda.</p>}<dl className="customer-amounts"><div><dt>{shared ? "Total tagihan gabungan" : "Total tagihan"}</dt><dd>{money(invoiceTotal(s, invoice.id))}</dd></div><div><dt>{shared ? "Sisa tagihan gabungan" : "Sisa pembayaran"}</dt><dd>{money(balance)}</dd></div></dl>
...
function CustomerAttachments({s, refresh, scope, targetId, readOnly = false}: Context & {scope: "payment" | "receipt" | "order"; targetId: string; readOnly?: boolean}) {
  ...
  async function upload(file: File) {
    ...
    uploading.current = true; setBusy(true); setError(""); setUploaded("");
    try { const data = new FormData(); data.set("file", file); data.set("scope", scope); data.set("targetId", targetId); await requestJson("/api/attachments", {method: "POST", body: data}); await refresh(); setUploaded(`${file.name} berhasil diunggah.`); }
...
function CustomerActionDialog({actorId, action, close, complete}: {actorId: string; action: CustomerAction; close: () => void; complete: (result: CommandResult) => Promise<void>}) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (action.type === "shipment.receive" && !draft && !reviewing) {
      setReviewing(true); setError(""); requestAnimationFrame(() => reviewRef.current?.focus()); return;
    }
  ...
  return <Dialog open onOpenChange={open => {if (!open && !pending.current) close();}}><DialogContent className="customer-action-dialog" onEscapeKeyDown={event => {if (pending.current) event.preventDefault();}}><DialogHeader><DialogTitle>{presentation.title}</DialogTitle><DialogDescription>{presentation.description}</DialogDescription></DialogHeader><form onSubmit={submit} aria-busy={busy}>
    ...
    {reviewing && !draft ? <div className="customer-receipt-review"><h3 ref={reviewRef} tabIndex={-1}>Periksa penerimaan Anda</h3><p>Penerima: <strong>{values.receiver}</strong></p><p>Pastikan jumlah sesuai barang yang benar-benar diterima.</p><ul>{presentation.fields.filter(field => field.key.startsWith("qty_")).map(field => {const received = Number(values[field.key]), sent = Number(field.max || 0), reason = values[`reason_${field.key.slice(4)}`]; return <li key={field.key}><strong>{field.label.replace(/ — diterima$/, "")}</strong><dl><div><dt>Dikirim</dt><dd>{sent}</dd></div><div><dt>Diterima</dt><dd>{received}</dd></div><div><dt>Selisih</dt><dd>{sent - received}</dd></div></dl>{reason && <p>Alasan: {reason}</p>}</li>;})}</ul></div> : <FieldGroup>{presentation.fields.map(f => <Field key={f.key} data-disabled={locked}><FieldLabel htmlFor={`customer-action-${f.key}`}>{f.label}</FieldLabel>{f.type === "textarea" ? <Textarea id={`customer-action-${f.key}`} value={values[f.key] || ""} disabled={locked} required={!f.optional} maxLength={1000} onChange={event => edit(f.key, event.target.value)}/> : <Input id={`customer-action-${f.key}`} type={f.type || "text"} min={f.min} max={f.max} step={f.type === "number" ? 1 : undefined} value={values[f.key] || ""} disabled={locked} required={!f.optional} maxLength={250} onChange={event => edit(f.key, event.target.value)}/>} {f.help && <FieldDescription>{f.help}</FieldDescription>}</Field>)}</FieldGroup>}
    {error && <Alert ref={errorRef} tabIndex={-1} variant="destructive"><AlertDescription>{error}</AlertDescription></Alert>}<DialogFooter><Button variant="outline" type="button" disabled={busy} onClick={() => {if (reviewing && !draft) {setReviewing(false); requestAnimationFrame(() => document.getElementById("customer-action-receiver")?.focus());} else close();}}>{draft ? "Tutup sementara" : reviewing ? "Ubah isian" : "Kembali"}</Button><Button type="submit" disabled={busy}>{busy ? "Menyimpan…" : draft ? "Periksa kembali" : action.type === "shipment.receive" ? reviewing ? "Konfirmasi penerimaan" : "Tinjau penerimaan" : presentation.label || "Simpan"}</Button></DialogFooter></form></DialogContent></Dialog>;
}

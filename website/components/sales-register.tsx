...
export function SalesRegister({ s, query, onQueryChange }: WorkspaceContext & OperationsQueryProps) {
  ...
  const buyers = buyerOptions(s);
  const division = buyers.some(d => d.value === query.get("salesDivision")) ? query.get("salesDivision")! : "all";
  ...
  const groups = buyers.filter(d => division === "all" || d.value === division).map(d => ({ name: d.label, value: sum(rows.filter(row => buyerFilterValue(row.order) === d.value).map(({ line }) => line.accepted * line.price)) }));
  ...
  return <section className="panel sales-register-v6"><div className="panel-title"><div><h2>Penjualan final</h2><p>Transaksi berdasarkan tanggal finalisasi penerimaan, sebelum nota kredit dan pajak invoice.</p></div><Button variant="outline" disabled={!validRange || !rows.length} onClick={download}><FileDown data-icon="inline-start"/>CSV sesuai filter</Button></div>
    ...
    {!!rows.length && <div className="sales-bars" aria-label="Nilai penjualan per pembeli">{groups.map(group => <div key={group.name}><span>{group.name}</span><div aria-hidden="true"><i style={{ width: `${group.value / max * 100}%` }}/></div><b>{money(group.value)}</b></div>)}</div>}
    {!!rows.length && <><div className="ops-desktop-table"><table className="data-table"><caption className="sr-only">Penjualan berdasarkan barang yang telah difinalkan pada rentang tanggal terpilih</caption><thead><tr><th scope="col">Tanggal / Surat Jalan</th><th scope="col">Pembeli / barang</th><th scope="col" className="numeric">Jumlah final</th><th scope="col" className="numeric">Nilai</th><th scope="col">Invoice</th></tr></thead><tbody>{shown.map(({ line, shipment, order, product, date }) => <tr key={line.id}><td>{date}<small>{shipment.number}</small></td><td><strong>{product.name}</strong><small>{product.sku} · {product.category}</small><small>{buyerLabel(s, order)}</small></td><td className="numeric">{line.accepted} {product.unit}</td><td className="numeric">{money(line.accepted * line.price)}</td><td>{invoiceLink(line.id)}</td></tr>)}</tbody></table></div><div className="ops-mobile-list">{shown.map(({ line, shipment, order, product, date }) => <article key={line.id} className="ops-record"><div className="ops-record-heading"><div><h3>{product.name}</h3><p>{product.sku} · {product.category}</p></div><strong>{money(line.accepted * line.price)}</strong></div><dl className="ops-facts"><div><dt>Finalisasi</dt><dd>{date}</dd></div><div><dt>Jumlah final</dt><dd>{line.accepted} {product.unit}</dd></div><div><dt>Pembeli</dt><dd>{buyerLabel(s, order)}</dd></div><div><dt>Surat Jalan</dt><dd>{shipment.number}</dd></div></dl>{invoiceLink(line.id)}</article>)}</div></>}
    {!rows.length && validRange && <Empty><EmptyHeader><EmptyTitle>Belum ada penjualan pada pilihan ini</EmptyTitle><EmptyDescription>Ubah rentang tanggal, sumber barang, atau pembeli. Penerimaan yang belum difinalisasi belum masuk laporan penjualan.</EmptyDescription></EmptyHeader></Empty>}

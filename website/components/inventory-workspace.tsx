...
function EmptyResults({ filtered, reset, title, action, children }: { filtered: boolean; reset: () => void; title: string; action?: ReactNode; children: ReactNode }) {
  return <Empty className="ops-empty"><EmptyHeader><PackageSearch aria-hidden="true"/><EmptyTitle>{filtered ? "Tidak ada hasil yang cocok" : title}</EmptyTitle><EmptyDescription>{filtered ? "Coba kata kunci lain atau hapus filter untuk melihat semua data." : children}</EmptyDescription></EmptyHeader>{filtered ? <Button variant="outline" onClick={reset}>Hapus filter</Button> : action}</Empty>;
}

function InventoryTable({ caption, heads, children }: { caption: string; heads: string[]; children: ReactNode }) {
  return <div className="inventory-desktop"><table className="data-table"><caption className="sr-only">{caption}</caption><thead><tr>{heads.map(head => <th scope="col" className={["Fisik / reservasi", "Tersedia", "Harga jual"].includes(head) ? "numeric" : undefined} key={head}>{head}</th>)}</tr></thead><tbody>{children}</tbody></table></div>;
}
...
export function InventoryWorkspace({ query, onQueryChange, ...ctx }: WorkspaceContext & OperationsQueryProps) {
  ...
  return <Tabs value={filters.tab} onValueChange={value => onQueryChange({ stockTab: value, stockPage: null, stockProduct: null })} className="inventory-v6 ops-workspace">
    ...
    <div className="ops-results-heading"><p role="status">{count} {tabLabels[filters.tab].toLowerCase()} ditemukan{filters.productId && ` untuk ${s.products.find(product => product.id === filters.productId)?.name || "barang terpilih"}`}</p>{filtered && count > 0 && <Button variant="ghost" size="sm" onClick={clear}>Hapus filter</Button>}</div>
    <TabsContent value="products">{products.length > 0 && <><InventoryTable caption="Stok dan kondisi persediaan menurut barang" heads={["Barang", "Fisik / reservasi", "Tersedia", "Kondisi", "Harga jual", "Tindakan"]}>{products.slice(offset, offset + pageSize).map(row => <tr key={row.product.id}><td><strong>{row.product.name}</strong><small>{row.product.sku} · {row.product.category} · {row.product.unit}</small></td><td className="numeric">{row.physical} / {row.reserved}<small>Ditahan {row.held}</small></td><td className="numeric"><strong>{row.available} {row.product.unit}</strong><small>Minimum {row.product.minimum}</small></td><td><ProductConditions row={row}/></td><td className="numeric">{money(row.product.price)}</td><td>{productActions(row)}</td></tr>)}</InventoryTable><div className="inventory-mobile">{products.slice(offset, offset + pageSize).map(row => <article className="ops-record" key={row.product.id}><div className="ops-record-heading"><div><h3>{row.product.name}</h3><p>{row.product.sku} · {row.product.category}</p></div><div className="inventory-available"><span>Tersedia</span><strong>{row.available} <small>{row.product.unit}</small></strong></div></div><ProductConditions row={row}/><dl className="ops-facts"><div><dt>Fisik / reservasi</dt><dd>{row.physical} / {row.reserved}</dd></div><div><dt>Harga jual</dt><dd>{money(row.product.price)}</dd></div></dl>{productActions(row)}</article>)}</div></>}</TabsContent>
    <TabsContent value="batches">{batches.length > 0 && <><InventoryTable caption="Stok, lokasi, dan masa kedaluwarsa menurut batch" heads={["Barang / batch", "Kedaluwarsa", "Fisik / reservasi", "Tersedia", "Tindakan"]}>{batches.slice(offset, offset + pageSize).map(row => <tr key={row.batch.id}><td><strong>{row.product.name}</strong><small>{row.product.sku} · {row.batch.code}</small><small>{row.batch.location}</small></td><td>{row.batch.expiry || "Tidak ditetapkan"}<BatchConditions row={row}/></td><td className="numeric">{row.batch.qty} / {row.reserved}<small>Ditahan {row.batch.held}</small></td><td className="numeric"><strong>{row.available} {row.product.unit}</strong></td><td><BatchActions ctx={ctx} row={row}/></td></tr>)}</InventoryTable><div className="inventory-mobile">{batches.slice(offset, offset + pageSize).map(row => <article className="ops-record" key={row.batch.id}><div className="ops-record-heading"><div><h3>{row.product.name}</h3><p>{row.product.sku} · {row.batch.code}</p></div><div className="inventory-available"><span>Tersedia</span><strong>{row.available} <small>{row.product.unit}</small></strong></div></div><BatchConditions row={row}/><dl className="ops-facts"><div><dt>Kedaluwarsa</dt><dd>{row.batch.expiry || "Tidak ditetapkan"}</dd></div><div><dt>Lokasi</dt><dd>{row.batch.location}</dd></div><div><dt>Fisik / reservasi</dt><dd>{row.batch.qty} / {row.reserved}</dd></div><div><dt>Ditahan</dt><dd>{row.batch.held} {row.product.unit}</dd></div></dl><BatchActions ctx={ctx} row={row}/></article>)}</div></>}</TabsContent>
    ...
    {!count && <EmptyResults filtered={filtered} reset={clear} title={filters.tab === "products" ? "Belum ada barang" : filters.tab === "batches" ? "Belum ada batch persediaan" : filters.tab === "returns" ? "Belum ada catatan retur" : "Belum ada penghitungan stok"} action={!filtered && filters.tab === "batches" && ["kepala", "staf"].includes(actor.role) ? <Button variant="outline" onClick={() => go("procurement")}>Lihat pengadaan</Button> : !filtered && ["returns", "stocktakes"].includes(filters.tab) && actor.role === "staf" && s.batches.length > 0 ? <Button variant="outline" onClick={() => onQueryChange({ stockTab: "batches", stockPage: null })}>Lihat batch barang</Button> : undefined}>{filters.tab === "products" ? actor.role === "kepala" ? "Gunakan Tambah barang untuk membuat katalog. Stok dicatat setelah penerimaan pengadaan." : "Kepala Toko perlu menambahkan barang sebelum persediaan dapat dicatat." : filters.tab === "batches" ? "Batch akan muncul setelah penerimaan pengadaan dicatat oleh Staf Toko." : filters.tab === "returns" ? actor.role === "staf" ? "Buka batch yang perlu diperiksa, lalu pilih Retur untuk memisahkan barang." : "Catatan akan muncul setelah Staf Toko memisahkan barang retur dari batch." : actor.role === "staf" ? "Buka batch dan pilih Opname untuk mencatat hasil penghitungan fisik." : actor.role === "kepala" ? "Penghitungan yang diajukan Staf Toko akan tampil di sini untuk ditinjau." : "Hasil penghitungan fisik dan keputusannya akan tampil setelah dicatat oleh tim toko."}</EmptyResults>}
    {count > 0 && <nav className="ops-pagination" aria-label="Halaman persediaan"><p>Menampilkan {offset + 1}–{Math.min(offset + pageSize, count)} dari {count}</p><div><Button variant="outline" disabled={page <= 1} onClick={() => onQueryChange({ stockPage: String(page - 1) })}>Sebelumnya</Button><span>Halaman {page} / {pages}</span><Button variant="outline" disabled={page >= pages} onClick={() => onQueryChange({ stockPage: String(page + 1) })}>Berikutnya</Button></div></nav>}
    </section>
  </Tabs>;
}

...
import { PAYMENT_STATUS_LABELS, paymentDisplayStatus, paymentListView, readPaymentQuery } from "@/lib/domain/payment-views-v15";
import {Art} from "./art";
...
function Empty({title,text,action}:{title:string;text:string;action?:{label:string;run:()=>void}}){return <div className="empty-state workspace-empty-v15"><Package size={28} aria-hidden="true"/><h3>{title}</h3><p>{text}</p>{action&&<Button variant="outline" onClick={action.run}>{action.label}</Button>}</div>;}
 ...
 return <div className="workspace"><Toaster richColors position="top-right"/>
  ...
  <div className="workspace-main"><header className="topbar"><button className="mobile-toggle" aria-label="Buka navigasi" aria-expanded={mobile} aria-controls="mobile-navigation" onClick={()=>setMobile(true)}><Menu size={22}/></button><div className="breadcrumb">Ruang kerja<ChevronRight size={14}/><strong>{active?.label||"Akses terbatas"}</strong></div><div className="topbar-right"><span className="demo-chip">DEMO</span><button className="account-mini" onClick={()=>go("profile")} aria-label="Buka profil saya"><Art src={actor.avatar||`/images/avatars/${actor.role==='pic'?'pic-a':actor.role}.png`} alt={`Foto ${actor.name}`}/><div><strong>{actor.name}</strong><small>{ROLE_LABELS[actor.role]}</small></div></button></div></header>
   <main className="content"><div className="page-top"><div><div className="eyebrow">{actor.role==="pic"?state.divisions.find(d=>d.id===actor.divisionId)?.name:"UNIT TOKO · KOPERASI"}</div><h1 id="workspace-title" tabIndex={-1}>{page==="dashboard"?"Ruang kerja Anda":active?.label||"Halaman tidak tersedia"}</h1><p>{active ? ROLE_PAGE_DESCRIPTIONS[actor.role]?.[page]||PAGE_DESCRIPTIONS[page] : "Gunakan menu yang tersedia untuk peran akun Anda."}</p></div><div className="page-tools"><span className="date-label">{new Date().toLocaleDateString("id-ID",{day:"numeric",month:"long",year:"numeric",timeZone:"Asia/Jakarta"})}</span><Button variant="outline" size="icon" aria-label={loading?"Memperbarui data":"Muat ulang data"} aria-busy={loading} disabled={loading} onClick={()=>void refresh()}><RefreshCw data-icon="inline-start"/></Button>{["pic","kepala"].includes(actor.role)&&page==="orders"&&<Button onClick={()=>go("catalog")}><Plus data-icon="inline-start"/>Buat pesanan</Button>}</div></div>
    ...
    {active&&page==="payments"&&<><Payments {...ctx} query={query} onQueryChange={updateQuery}/><SettlementControls {...ctx}/><ExpenseCenter {...ctx}/></>}
    {active&&page==="procurement"&&<Procurement {...ctx} query={query} onQueryChange={updateQuery}/>}
    {active&&page==="reports"&&<Reports {...ctx} query={query} onQueryChange={updateQuery}/>}
    {active&&page==="periods"&&<Periods {...ctx}/>}
    {active&&page==="admin"&&<AccountSettings {...ctx} query={query} onQueryChange={updateQuery}/>}
...
function Deliveries(ctx:WorkspaceContext&{selected?:string;query:URLSearchParams;onQueryChange:(patch:QueryPatch)=>void}){
 ...
 const status=["all","active","ready","dispatched","received","failed","cancelled"].includes(rawStatus)?rawStatus:"all";
 ...
 return <>{!selected&&<ListFilter id="delivery-filter" label="Status pengiriman" value={status} onChange={value=>onQueryChange({status:value==="all"?null:value})} options={[{value:"all",label:"Semua pengiriman"},{value:"active",label:"Pengiriman aktif"},{value:"ready",label:"Siap dikirim"},{value:"dispatched",label:"Dalam perjalanan"},{value:"received",label:"Diterima"},{value:"failed",label:"Gagal dikirim"},{value:"cancelled",label:"Dibatalkan"}]}/>}{selected&&<button className="back-link" onClick={()=>go("deliveries")}><ArrowLeft size={16}/>Semua pengiriman</button>}{shipments.map(sh=>{const o=get(s.orders,sh.orderId),ls=s.shipmentLines.filter(l=>l.shipmentId===sh.id),complaints=s.complaints.filter(c=>c.shipmentId===sh.id);return <section className="panel delivery-card" key={sh.id}><div className="panel-title"><div><h2>{sh.number}</h2><p>{buyerLabel(s,o)} · {o.address}</p></div><Status text={shipmentLabel(sh.status)}/></div><div className="delivery-meta"><span><Truck size={16}/>{sh.vehicle}</span><span>{s.users.find(u=>u.id===sh.courierId)?.name||"Kurir toko"}</span><span>{shortDate(sh.date)}</span><span>Pesanan {o.number}</span></div><div className="table-scroll"><table className="data-table"><caption className="sr-only">Rincian barang pada surat jalan {sh.number}</caption><thead><tr><th scope="col">Barang</th><th scope="col" className="numeric">Jumlah Surat Jalan</th><th scope="col" className="numeric">Diterima</th><th scope="col" className="numeric">Kembali</th><th scope="col">Final</th></tr></thead><tbody>{ls.map(l=><tr key={l.id}><td>{get(s.products,get(s.orderLines,l.orderLineId).productId).name}</td><td className="numeric">{l.qty}</td><td className="numeric">{l.accepted}</td><td className="numeric">{l.returned}</td><td>{l.finalized?"Ya":"Belum"}</td></tr>)}</tbody></table></div>{sh.receiver&&<p className="delivery-proof">Penerima: <strong>{sh.receiver}</strong> · {sh.evidence}</p>}
 ...
 </section>;})}{!shipments.length&&<section className="panel"><Empty title={selected?"Pengiriman tidak tersedia":s.shipments.length&&status!=="all"?"Tidak ada pengiriman pada status ini":"Belum ada tugas pengiriman"} text={selected?"Surat jalan tidak ditemukan atau tidak dapat diakses oleh akun Anda.":s.shipments.length&&status!=="all"?"Pilih status lain atau hapus filter untuk melihat pengiriman yang tersedia.":actor.role==="kurir"?"Pengiriman akan tampil setelah Staf Toko menugaskan surat jalan kepada Anda.":"Surat jalan akan tampil setelah barang disiapkan dan pengiriman dibuat."} action={selected?{label:"Lihat semua pengiriman",run:()=>go("deliveries",undefined,{status:null})}:status!=="all"?{label:"Hapus filter",run:()=>onQueryChange({status:null})}:undefined}/></section>}<ExpenseCenter {...ctx}/></>;}
function Billing(ctx:WorkspaceContext&{selected?:string;query:URLSearchParams;onQueryChange:(patch:QueryPatch)=>void}){
 ...
 const rawBalance=query.get("balance")||"all";
 const balance=["all","open","paid"].includes(rawBalance)?rawBalance:"all";
 ...
 return <>
  ...
  <section className="panel"><div className="panel-title"><h2>Invoice pembeli</h2><span className="muted">Sisa piutang {money(sum(invoices.map(i=>invoiceBalance(s,i.id))))}</span></div>
   <div className="table-scroll invoice-desktop"><table className="data-table"><caption className="sr-only">Invoice pembeli, jatuh tempo, dan sisa piutang</caption><thead><tr><th scope="col">Invoice / pembeli</th><th scope="col">Jatuh tempo</th><th scope="col" className="numeric">Total</th><th scope="col" className="numeric">Sisa piutang</th><th scope="col">Status / dokumen</th></tr></thead><tbody>{invoices.map(i=><tr key={i.id} id={`invoice-${i.id}`}><td><strong>{i.number}</strong><small>{buyerLabel(s,i)}</small></td><td>{shortDate(i.dueDate)}</td><td className="numeric">{money(invoiceTotal(s,i.id))}</td><td className="numeric">{money(invoiceBalance(s,i.id))}</td><td><Status text={invoiceStatus(s,i)}/><DocumentLink kind="invoice" id={i.id}/></td></tr>)}</tbody></table></div>
   ...
   {!invoices.length&&<Empty title={selected?"Invoice tidak tersedia":s.invoices.length&&balance!=="all"?"Tidak ada invoice pada status ini":"Belum ada invoice"} text={selected?"Invoice tidak ditemukan atau tidak dapat diakses oleh akun Anda.":s.invoices.length&&balance!=="all"?"Hapus filter untuk melihat seluruh invoice yang tersedia.":"Bagian Penagihan menerbitkan invoice setelah penerimaan barang difinalisasi."} action={selected?{label:"Lihat semua invoice",run:()=>go("billing",undefined,{balance:null,returnOrder:null})}:balance!=="all"?{label:"Hapus filter",run:()=>onQueryChange({balance:null})}:undefined}/>}</section>
  {composing&&<InvoiceComposer ctx={ctx} close={()=>setComposing(false)}/>}
 </>;
}
function Payments(ctx:WorkspaceContext&{query:URLSearchParams;onQueryChange:(patch:QueryPatch)=>void}){const {s,actor,ask,query,onQueryChange}=ctx;const filters=readPaymentQuery(query),view=paymentListView(s,filters);const clear=()=>onQueryChange({paymentSearch:null,paymentStatus:null,paymentPage:null});const [allocationId,setAllocationId]=useState<string|null>(null);const buyers=buyerOptions(s);const buyerRef=(value:string)=>buyers.find(b=>b.value===value)?.ref||{divisionId:null};function record(){ask({title:"Catat transfer simulasi",description:"Pencatatan ini tidak mengirim uang. Piutang berkurang setelah verifikasi dan alokasi oleh Penagihan.",type:"payment.record",fields:[...(actor.role==="penagihan"?[{key:"buyer",label:"Pembeli (kosongkan jika belum diketahui)",type:"select" as const,optional:true,options:buyers}]:[]),{key:"amount",label:"Nilai transfer (Rp)",type:"number"},{key:"payer",label:"Nama pembayar",value:actor.name},{key:"reference",label:"Referensi transfer",optional:true},{key:"note",label:"Catatan",type:"textarea",optional:true}],map:v=>({...buyerRef(v.buyer),amount:Number(v.amount),payer:v.payer,reference:v.reference,note:v.note}),label:"Catat transfer"});}
 return <><section className="panel payments-v15"><div className="panel-title"><div><h2>Penerimaan dana</h2><p>Bukti transfer perlu diverifikasi sebelum pelunasan.</p></div>{["pic","penagihan"].includes(actor.role)&&<Button onClick={record}><Plus data-icon="inline-start"/>Catat transfer</Button>}</div><FieldGroup className="workspace-list-controls-v15">
 <Field><FieldLabel htmlFor="payment-search">Cari pembayaran</FieldLabel><Input id="payment-search" type="search" placeholder="Pembayar, referensi, atau pembeli" value={filters.search} onChange={event=>onQueryChange({paymentSearch:event.target.value||null,paymentPage:null})}/></Field>
 <Field><FieldLabel htmlFor="payment-status">Status pembayaran</FieldLabel><select id="payment-status" className="native-select" value={filters.status} onChange={event=>onQueryChange({paymentStatus:event.target.value==="all"?null:event.target.value,paymentPage:null})}>{Object.entries(PAYMENT_STATUS_LABELS).map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></Field>
 </FieldGroup><div className="workspace-list-results-v15"><p role="status">{view.count} pembayaran{view.filtered?` dari ${view.total} catatan`:""}</p>{view.filtered&&view.count>0&&<Button variant="ghost" size="sm" onClick={clear}>Hapus filter</Button>}</div>
 {!!view.count&&<div className="table-scroll" role="region" aria-label="Tabel penerimaan dana" tabIndex={0}><table className="data-table"><caption className="sr-only">Penerimaan dana sesuai pencarian dan status pilihan. Halaman {view.page} dari {view.pages}.</caption><thead><tr><th scope="col">Pembayar / referensi</th><th scope="col" className="numeric">Jumlah</th><th scope="col" className="numeric">Belum dialokasikan</th><th scope="col">Status</th>{actor.role==="penagihan"&&<th scope="col"><span className="sr-only">Tindakan</span></th>}</tr></thead><tbody>{view.items.map(p=><tr key={p.id}><td><strong>{p.payer}</strong><small>{p.reference||"Tanpa referensi"} · {shortDate(p.date)}</small><small>{buyerLabel(s,p)}</small><Attachments ctx={ctx} scope="payment" targetId={p.id}/></td><td className="numeric">{money(p.amount)}</td><td className="numeric">{p.status==="verified"?money(paymentAvailable(s,p.id)):"—"}</td><td><Status text={PAYMENT_STATUS_LABELS[paymentDisplayStatus(p)]}/>{p.status==="rejected"&&<p className="module-note"><strong>Alasan penolakan</strong><br/>{p.rejectionReason}<br/>Diputuskan {shortDate(p.rejectedDate||p.date)}. Catat ulang pembayaran dengan data yang benar.</p>}</td>{actor.role==="penagihan"&&<td><div className="row-actions">{p.status==="recorded"&&<><Button variant="outline" size="sm" onClick={()=>ask({title:"Verifikasi dana masuk",description:"Pastikan transfer simulasi telah cocok dengan data penerimaan dana.",type:"payment.verify",data:{id:p.id},fields:[],label:"Verifikasi dana"})}>Verifikasi</Button><Button variant="outline" size="sm" onClick={()=>ask({title:"Tolak bukti pembayaran",description:"Pembayaran ini tidak masuk saldo atau pelunasan. Alasan dapat dilihat pembayar agar catatan yang benar dapat diajukan kembali.",type:"payment.reject",data:{id:p.id},fields:[{key:"reason",label:"Alasan penolakan",type:"textarea"}],label:"Tolak pembayaran"})}>Tolak</Button></>}{p.status!=="rejected"&&!buyerKey(p)&&<Button variant="outline" size="sm" onClick={()=>ask({title:"Identifikasi pembayar",description:"Catat hasil penelusuran dan pembeli pemilik dana.",type:"payment.identify",data:{id:p.id},fields:[{key:"buyer",label:"Pembeli",type:"select",options:buyers},{key:"reason",label:"Hasil penelusuran",type:"textarea"}],map:v=>({...buyerRef(v.buyer),reason:v.reason})})}>Identifikasi</Button>}{p.status==="verified"&&buyerKey(p)&&paymentAvailable(s,p.id)>0&&<Button size="sm" onClick={()=>setAllocationId(p.id)}>Alokasikan</Button>}</div></td>}</tr>)}</tbody></table></div>}
 {!view.count&&<Empty title={view.total&&view.filtered?"Tidak ada pembayaran yang cocok":"Belum ada catatan pembayaran"} text={view.total&&view.filtered?"Coba nama atau referensi lain, atau hapus filter untuk melihat seluruh catatan pembayaran.":["pic","penagihan"].includes(actor.role)?"Gunakan Catat transfer untuk menyimpan pembayaran simulasi beserta bukti pendukungnya.":"Catatan akan tampil setelah pembayar atau Bagian Penagihan mencatat penerimaan dana."} action={view.filtered?{label:"Hapus filter",run:clear}:undefined}/>}
 {view.count>0&&<nav className="ops-pagination workspace-pagination-v15" aria-label="Halaman penerimaan dana"><p>Menampilkan {view.start}–{view.end} dari {view.count} pembayaran</p>{view.pages>1&&<div><Button variant="outline" disabled={view.page===1} onClick={()=>onQueryChange({paymentPage:String(view.page-1)})}>Sebelumnya</Button><span>Halaman {view.page} / {view.pages}</span><Button variant="outline" disabled={view.page===view.pages} onClick={()=>onQueryChange({paymentPage:String(view.page+1)})}>Berikutnya</Button></div>}</nav>}
 </section>{allocationId&&<PaymentAllocation ctx={ctx} paymentId={allocationId} close={()=>setAllocationId(null)}/>}</>;
}

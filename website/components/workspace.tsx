...
const PAGE_DESCRIPTIONS:Record<Page,string>={
 ...
 billing:"Pantau tagihan, jatuh tempo, dan sisa piutang divisi.",
 payments:"Tinjau pembayaran, saldo divisi, dan pengembalian dana.",
...
export function Workspace(){
 ...
 return <div className="workspace"><Toaster richColors position="top-right"/>
  ...
  <div className="workspace-main"><header className="topbar"><button className="mobile-toggle" aria-label="Buka navigasi" aria-expanded={mobile} aria-controls="mobile-navigation" onClick={()=>setMobile(true)}><Menu size={22}/></button><div className="breadcrumb">Ruang kerja<ChevronRight size={14}/><strong>{active?.label||"Ringkasan"}</strong></div><div className="topbar-right"><span className="demo-chip">DEMO</span><button className="account-mini" onClick={()=>go("profile")} aria-label="Buka profil saya"><Art src={actor.avatar||`/images/avatars/${actor.role==='pic'?'pic-a':actor.role}.png`} alt={`Foto ${actor.name}`}/><div><strong>{actor.name}</strong><small>{ROLE_LABELS[actor.role]}</small></div></button></div></header>
   <main className="content"><div className="page-top"><div><div className="eyebrow">{actor.role==="pic"?state.divisions.find(d=>d.id===actor.divisionId)?.name:"UNIT TOKO · KOPERASI"}</div><h1 id="workspace-title" tabIndex={-1}>{page==="dashboard"?"Ringkasan":active?.label}</h1><p>{ROLE_PAGE_DESCRIPTIONS[actor.role]?.[page]||PAGE_DESCRIPTIONS[page]}</p></div><div className="page-tools"><span className="date-label">{new Date().toLocaleDateString("id-ID",{day:"numeric",month:"long",year:"numeric",timeZone:"Asia/Jakarta"})}</span><Button variant="outline" size="icon" aria-label={loading?"Memperbarui data":"Muat ulang data"} aria-busy={loading} disabled={loading} onClick={()=>void refresh()}><RefreshCw data-icon="inline-start"/></Button>{["pic","kepala"].includes(actor.role)&&["dashboard","orders"].includes(page)&&<Button onClick={()=>go("catalog")}><Plus data-icon="inline-start"/>Buat pesanan</Button>}</div></div>
...
function Payments(ctx:WorkspaceContext){const {s,actor,ask}=ctx;const [allocationId,setAllocationId]=useState<string|null>(null);function record(){ask({title:"Catat transfer simulasi",description:"Pencatatan ini tidak mengirim uang. Piutang berkurang setelah verifikasi dan alokasi oleh Penagihan.",type:"payment.record",fields:[...(actor.role==="penagihan"?[{key:"divisionId",label:"Divisi (kosongkan jika belum diketahui)",type:"select" as const,optional:true,options:s.divisions.map(d=>({value:d.id,label:d.name}))}]:[]),{key:"amount",label:"Nilai transfer (Rp)",type:"number"},{key:"payer",label:"Nama pembayar",value:actor.name},{key:"reference",label:"Referensi transfer",optional:true},{key:"note",label:"Catatan",type:"textarea",optional:true}],label:"Catat transfer"});}
 return <><section className="panel"><div className="panel-title"><div><h2>Penerimaan dana</h2><p>Bukti transfer perlu diverifikasi sebelum pelunasan.</p></div>{["pic","penagihan"].includes(actor.role)&&<Button onClick={record}><Plus data-icon="inline-start"/>Catat transfer</Button>}</div><div className="table-scroll"><table className="data-table"><thead><tr><th>Pembayar / referensi</th><th>Jumlah</th><th>Belum dialokasikan</th><th>Status</th><th/></tr></thead><tbody>{[...s.payments].reverse().map(p=><tr key={p.id}><td><strong>{p.payer}</strong><small>{p.reference||"Tanpa referensi"} · {shortDate(p.date)}</small><small>{s.divisions.find(d=>d.id===p.divisionId)?.name||"Divisi belum diketahui"}</small><Attachments ctx={ctx} scope="payment" targetId={p.id}/></td><td className="numeric">{money(p.amount)}</td><td>{p.status==="verified"?money(paymentAvailable(s,p.id)):"—"}</td><td><Status text={p.status==="recorded"?"Menunggu verifikasi":p.divisionId?"Terverifikasi":"Belum teridentifikasi"}/></td><td>{actor.role==="penagihan"&&<div className="row-actions">{p.status==="recorded"&&<Button variant="outline" size="sm" onClick={()=>ask({title:"Verifikasi dana masuk",description:"Pastikan transfer simulasi telah cocok dengan data penerimaan dana.",type:"payment.verify",data:{id:p.id},fields:[],label:"Verifikasi dana"})}>Verifikasi</Button>}{!p.divisionId&&<Button variant="outline" size="sm" onClick={()=>ask({title:"Identifikasi divisi pembayar",description:"Catat hasil penelusuran dan divisi pemilik dana.",type:"payment.identify",data:{id:p.id},fields:[{key:"divisionId",label:"Divisi pembayar",type:"select",options:s.divisions.map(d=>({value:d.id,label:d.name}))},{key:"reason",label:"Hasil penelusuran",type:"textarea"}]})}>Identifikasi</Button>}{p.status==="verified"&&p.divisionId&&paymentAvailable(s,p.id)>0&&<Button size="sm" onClick={()=>setAllocationId(p.id)}>Alokasikan</Button>}</div>}</td></tr>)}</tbody></table></div>{!s.payments.length&&<Empty text="Belum ada catatan transfer."/>}</section>{allocationId&&<PaymentAllocation ctx={ctx} paymentId={allocationId} close={()=>setAllocationId(null)}/>}</>;
}

...
import {ROLES, isRole, type Actor, type Role} from "./accounts";
import {assertBuyerAccess, buyerKey, buyerRefForActor, sameBuyer} from "./buyers";
...
export function allowed(actor:Actor,roles:readonly Role[]){if(!actor.active||!isRole(actor.role)||!roles.includes(actor.role))throw new DomainError("Peran Anda tidak memiliki akses untuk tindakan ini.",403);}
export function ownDivision(actor:Actor,divisionId:string|null){assertBuyerAccess(actor,{divisionId});}
...
export function scopeState(s:State,actor:Actor):State {
  ...
  if(actor.role!=="pic"&&actor.role!=="kurir"&&actor.role!=="customer"){
    ...
    if(actor.role!=="staf"){
      ...
      if(actor.role==="kepala"||actor.role==="laporan")copy.payments=copy.payments.map(p=>({...p,reference:"",payer:buyerKey(p)?"Pembayaran pembeli":"Dana belum diidentifikasi",note:"",evidence:"",createdBy:"",verifiedBy:null}));
    }
  ...
  const buyer=actor.role==="pic"||actor.role==="customer"?buyerRefForActor(actor):null;
  const orderIds=new Set(buyer?s.orders.filter(o=>sameBuyer(o,buyer)).map(o=>o.id):s.shipments.filter(x=>x.courierId===actor.id).map(x=>x.orderId));
  ...
  copy.invoices=buyer?s.invoices.filter(x=>sameBuyer(x,buyer)):[];const invoiceIds=new Set(copy.invoices.map(x=>x.id));copy.invoiceLines=s.invoiceLines.filter(x=>invoiceIds.has(x.invoiceId));copy.credits=s.credits.filter(x=>invoiceIds.has(x.invoiceId));
  copy.payments=buyer?s.payments.filter(x=>sameBuyer(x,buyer)):[];const paymentIds=new Set(copy.payments.map(x=>x.id));copy.allocations=s.allocations.filter(x=>invoiceIds.has(x.invoiceId)&&paymentIds.has(x.paymentId));copy.refunds=s.refunds.filter(x=>paymentIds.has(x.paymentId));
  copy.divisions=s.divisions.filter(d=>actor.role==="pic"?d.id===actor.divisionId:actor.role==="customer"?false:copy.orders.some(o=>o.divisionId===d.id));copy.users=s.users.filter(u=>u.id===actor.id);

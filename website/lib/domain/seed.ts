...
export function seedState(){
  let s=emptyState();const date=today();s.users=DEMO_ACCOUNTS.map(({id,name,role,divisionId,active,email,address})=>({id,name,role,divisionId,active,email:email||`${id}@unit-toko.demo`,avatar:role==="customer"?"":`/images/avatars/${id}.png`,phone:"",position:role==="customer"?"":role==="pic"?"Penanggung jawab kebutuhan divisi":"Tim Unit Toko",...(role==="customer"?{address:address||""}:{})}));

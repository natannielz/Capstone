...
export const TABLES: [Collection,string,string[]][] = [
  ["divisions","divisions",[]],["users","users",["divisionId","role"]],["products","products",["sku"]],["suppliers","suppliers",[]],["batches","batches",["productId","qty","held"]],["orders","orders",["divisionId","number"]],["orderLines","order_lines",["orderId","productId"]],["reservations","reservations",["orderLineId","batchId","qty"]],["substitutions","substitutions",["orderLineId","productId"]],["shipments","shipments",["orderId","courierId","number"]],["shipmentLines","shipment_lines",["shipmentId","orderLineId","batchId"]],["complaints","complaints",["shipmentId","shipmentLineId","divisionId"]],["invoices","invoices",["divisionId","number"]],["invoiceLines","invoice_lines",["invoiceId","shipmentLineId"]],["payments","payments",["divisionId"]],["allocations","allocations",["paymentId","invoiceId"]],["credits","credits",["invoiceId"]],["refunds","refunds",["paymentId"]],["purchases","purchases",["supplierId","orderId","number"]],["purchaseLines","purchase_lines",["purchaseId","productId"]],["supplierPayments","supplier_payments",["purchaseId"]],["stockReturns","stock_returns",["batchId"]],["stocktakes","stocktakes",["batchId"]],["expenses","expenses",["shipmentId","purchaseId"]],["movements","movements",["batchId"]],["journals","journals",[]],["journalLines","journal_lines",["journalId","debit","credit"]],["periods","periods",[]],["attachments","attachments",["ownerId","divisionId","shipmentId"]],["audits","audits",["actorId"]]
];
...
export async function ensureSeed(){if(initializing)return initializing;initializing=(async()=>{await initializeDatabase();const db=database();if(await db.prepare("SELECT version FROM system_version WHERE id='global'").first()){await upgradeProfiles(db);return;}const s=seedState();const queries=[db.prepare("INSERT INTO system_version(id,version) VALUES('global',?)").bind(s.revision)];for(const [key,table,keys] of TABLES)for(const item of s[key])queries.push(writeRecord(db,table,keys,item));for(const user of s.users){const salt=crypto.randomUUID(),hash=await hashPassword(demoPassword(user.id),salt);queries.push(db.prepare("INSERT INTO user_emails(email,user_id) VALUES(?,?)").bind(user.email,user.id));queries.push(db.prepare("INSERT INTO credentials(user_id,salt,hash) VALUES(?,?,?)").bind(user.id,salt,hash));}try{await db.batch(queries);}catch(err){if(!await db.prepare("SELECT version FROM system_version WHERE id='global'").first())throw err;}})();try{await initializing;}catch(err){initializing=undefined;throw err;}}
